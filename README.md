...
| `new_cert_events_per_second` | `50` | Caps how many never-before-seen certificate files can be fully parsed per second, across all trigger paths (real-time Tetragon events, periodic scan, and the large-file background thread all share one budget). Bounds the CPU an attacker can force purely through certificate activity (e.g. writing many distinct cert files) with no config access needed — the cap is per distinct new *path*, not per certificate, so a single large bundle (even hundreds of certs) still only costs one token. A throttled file isn't lost — it's simply skipped for that event and gets another chance on a later access or the next periodic scan. Set to `0` to disable |
