...
![Build](https://github.com/bensanmorris/security_observability/actions/workflows/build.yml/badge.svg)
[![codecov](https://codecov.io/gh/bensanmorris/security_observability/branch/main/graph/badge.svg)](https://codecov.io/gh/bensanmorris/security_observability)
...
| [MCP server](extras/mcp-server/MCP-SERVER-README.md) (read-only) | `claude mcp add --transport http certsight http://certsight-demo.com:8092/mcp` | Query fleet cert/FIPS/chain data live from Claude Desktop or Claude Code - no auth needed (not a browser link). Try asking:<ul><li>*"which nodes haven't finished the FIPS migration?"*</li><li>*"what's expiring in the next 7 days?"*</li><li>*"what's the blast radius if this cert's key is compromised?"*</li><li>*"explain this cert's chain - is an intermediate missing?"*</li></ul><br>[Setup instructions](extras/mcp-server/MCP-SERVER-README.md#running-over-the-network-streamable-http) |
| [Fleet manager](extras/fleet-manager/FLEET-MANAGER-README.md) (read-only) | [certsight-demo.com:8094](http://certsight-demo.com:8094) | Fleet policy console: every node, a nodes × policies matrix with each node's live Tetragon policy state, and the fleet explorers in one place. Click *Continue as read-only viewer* - the demo has no admin account, so nothing here can change a node; on your own fleet an admin can switch a policy on or off per node or fleet-wide and the node keeps that decision across Tetragon restarts |
...
| **CertSight** | • Every certificate file access system-wide <br>• In-memory certificates post-handshake <br>• Dynamically linked crypto <br>• Java certificate operations in both FIPS and non-FIPS environments (verified on Java 8, 11, 17, 21, and 25) <br>• Certs on open ports via optional port probe (triggered by bind events, no port sweep required) <br>• Remote server certificates via outbound connect probe (triggered by `tcp_connect` to TLS ports, no configuration required) - optionally captures the real SNI hostname a client's own OpenSSL library sends and probes with it instead of the raw destination IP so an SNI-multiplexed CDN edge (Fastly, Cloudflare, CloudFront, etc) returns the real vhost certificate instead of its generic default (opt-in - see `sni_capture_enabled` below) <br><br> Every cert access is FIPS compliance checked with full process and k8s context surfaced <br><br> Extracts an extensive set of surfaced fields and metrics per certificate access. Refer to our [surfaced fields guide](extras/FIELDS-README.md) or our [Kafka schema](https://github.com/bensanmorris/security_observability/blob/main/extras/FIELDS-README.md#kafka-event-schema) <br><br> Also ships with a [Grafana dashboard](extras/DASHBOARDS.md) and a [test console](extras/test-server/TEST-SERVER-README.md) | Statically linked crypto (excluding Java / JCA - we've got that covered)
...
| Setting | Default | Description |
|---|---|---|
| `enabled` | `false` | Start the control listener. Routes: `GET /control/info`, `GET /control/policies`, `PUT /control/policies/<name>` (`{"enabled": true\|false}`, `?namespace=` for namespaced k8s policies). Cannot add or remove policies |
| `listen` | `127.0.0.1:8087` | Bind address. Reach a loopback listener from elsewhere via an SSH tunnel, or bind a management interface explicitly — with TLS, or cert-analyzer warns that the token crosses the network in clear |
| `allowed_sources` | _(unset)_ | Comma-separated CIDRs allowed to connect; checked before authentication, refused callers get a bodiless 403 |
| `token` | _(unset)_ | Shared bearer secret (≥16 chars). Required unless `tls_client_ca` is set; if both are set, both are required |
| `tls_cert`, `tls_key` | _(unset)_ | Serve HTTPS (TLS 1.2+). Must be set together |
| `tls_client_ca` | _(unset)_ | Require a client certificate signed by this CA (mutual TLS); needs `tls_cert`/`tls_key` |
| `authorized_clients` | _(unset)_ | Client-certificate CNs allowed to change policies. With `tls_client_ca` set and both CN lists empty, any CA-signed client is an operator (warned at startup) |
...
| `new_cert_events_per_second` | `50` | Caps how many never-before-seen certificate files can be fully parsed per second, across all trigger paths (real-time Tetragon events, periodic scan, the large-file background thread, and the retry queue below all share one budget). Bounds the CPU an attacker can force purely through certificate activity (e.g. writing many distinct cert files) with no config access needed — the cap is per distinct new *path*, not per certificate, so a single large bundle (even hundreds of certs) still only costs one token. A throttled file isn't lost — it's queued and replayed once capacity frees up (see `retry_queue_max_size`). Set to `0` to disable |
| `retry_queue_max_size` | `2000` | Bounds the rate-limit retry queue — a throttled file is queued here (with its original triggering process/pod context intact) and replayed by a dedicated background thread as `new_cert_events_per_second` capacity frees up. This is what makes "throttled but not lost" true regardless of whether the file happens to fall under `[scanning] paths` — periodic scan alone only recovers files under its configured scan paths. Bounded FIFO: at capacity, the oldest queued entry is dropped to make room, so this can't become a second unbounded memory sink for the same abuse the rate limiter itself defends against |
...
| `probe_events_per_second` | `20` | Caps how many *new* bind/connect probe threads can be started per second, regardless of how large a single burst of kprobe events is — e.g. a package manager retrying several mirror IPs on the same TLS port in quick succession. A probe throttled here is dropped, same as one that loses the `max_concurrent_background_threads` race — there is no retry queue. Matches `max_concurrent_background_threads`'s default (20) so it costs no coverage beyond what that thread cap already drops on its own, while still bounding *sustained* throughput over time. Lowering this trades away burst coverage roughly linearly for CPU — there's no sweet spot, it's a straight tradeoff. Set to `0` to disable |
| `sni_capture_enabled` | `false` | Outbound-only (`connect_probe_enabled`; has no effect on `bind_probe_enabled`) — dial with the real hostname a client's own OpenSSL library was about to send as SNI, captured via an `SSL_ctrl` uprobe, instead of the raw destination IP. Fixes SNI-multiplexed CDN edges (Fastly, Cloudflare, CloudFront, …) returning their generic fallback certificate instead of the real one an IP-string SNI matches no vhost on. Requires the `SSL_ctrl` uprobe in `openssl3-cert-load.yaml` to be loaded (RHEL9-class kernels only — string uprobe args need kernel 6.3+, no effect on RHEL8). Off by default: statically-linked/non-OpenSSL clients never call `SSL_ctrl` so they still fall back to the raw-IP behavior either way, and correlation is by PID alone, so a process with several concurrent outbound TLS connections at once can have hostnames cross-attributed |
| `sni_capture_window_seconds` | `2` | How many seconds a captured SNI hostname (see `sni_capture_enabled` above) stays valid for connect-probe to use. Too short and a legitimately slow handshake misses the window and falls back to the raw IP; too long risks using a stale hostname if the same PID opens another outbound connection to a different host before this one's probe runs |
