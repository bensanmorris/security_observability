...
| **CertSight** | Every certificate file access system-wide + In-memory certificates post-handshake + Dynamically linked crypto with every cert access FIPS compliance checked with full process and k8s context | Statically linked crypto (excluding Java / JCA - we've got that covered) |
