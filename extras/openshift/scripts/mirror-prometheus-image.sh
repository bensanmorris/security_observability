...
echo
echo "Use it with the cert-analyzer Helm chart (demo.prometheus.image is registry/repository/tag,"
echo "not a single field -- if cert-analyzer/cert-test-server share this same registry host,"
echo "set global.imageRegistry once instead and drop demo.prometheus.image.registry below):"
echo "  helm upgrade --install cert-analyzer ./extras/helm/cert-analyzer -n $NAMESPACE \\"
echo "    --reuse-values \\"
echo "    --set demo.prometheus.enabled=true \\"
echo "    --set demo.prometheus.image.registry=image-registry.openshift-image-registry.svc:5000 \\"
echo "    --set demo.prometheus.image.repository=$NAMESPACE/prometheus \\"
echo "    --set demo.prometheus.image.tag=$TAG"
