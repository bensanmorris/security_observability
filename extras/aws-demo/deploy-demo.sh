...
#
# The dashboard (Grafana, port 3000), test console (port 8090), and MCP
# server (port 8092) are all opened to the *entire internet*, unauthenticated,
# by design -- this is a demo box, not a production deployment. The MCP
# server is rate-limited by nginx the same way the test console is -- see
...
if [[ -z "${SG_ID}" || "${SG_ID}" == "None" ]]; then
    ...
    aws ec2 authorize-security-group-ingress --region "${AWS_REGION}" --group-id "${SG_ID}" \
        ...
        "IpProtocol=tcp,FromPort=8094,ToPort=8094,IpRanges=[{CidrIp=0.0.0.0/0,Description='CertSight fleet manager (read-only, no admin account)'}]" \
...
echo " Fleet manager: http://${LINK_HOST}:8094   read-only (no admin account:"
echo "                nothing on it can change a node)"
echo ""
echo " All of the above are open to the internet with no authentication;"
echo " none of them can change what a node detects."
