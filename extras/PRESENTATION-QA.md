...
**Is this a "ring 0" product?**
Yes and no, and the distinction matters more than the label. Yes — eBPF programs are JIT-compiled and genuinely execute in kernel context (ring 0, not ring 3); that's what lets them hook every `fd_install`/`tcp_connect` on a busy host without userspace syscall-trampoline overhead. No, in the sense the question is usually really probing — "does this carry the risk profile of a traditional ring-0 component like a kernel module or driver?" It doesn't, for two structural reasons rather than just policy: (1) the eBPF verifier statically proves every program safe before load — bounded loops, no arbitrary pointer dereference, no way to crash or corrupt the kernel the way a buggy LKM can, and this is an eBPF/kernel guarantee, not something Tetragon or CertSight has to additionally get right; (2) every `TracingPolicy` in this repo (`tetragon-policies/*.yaml`) uses `kprobes:`/`uprobes:` attach points only — never `XDP` or `tc` classifier program types, which are the eBPF categories that *can* rewrite or drop packets — so these programs only copy argument bytes out via `bpf_probe_read` into a perf/ring buffer for userspace to read; they never write into kernel data structures, redirect a syscall, or touch a packet in flight. Tighter one-liner: it runs in kernel space, but as a passive, verifier-proven observer — closer in risk profile to a read-only kernel tracepoint than to a kernel module.
...
**Do the `openssl3-cert-load.yaml` uprobes (in-memory ASN.1 cert load, SNI capture) work against a containerized workload's own OpenSSL calls?**
Known limitation — confirmed broken 2026-09-07 on the AWS demo k8s node. The chart's container variant of this policy (`extras/helm/cert-analyzer/templates/policies/openssl3-cert-load.yaml`) points its `path` at `/procRoot/1/root/usr/lib64/libssl.so.3`, i.e. PID 1's root filesystem — the **node's own host libssl**, reached the same way Tetragon resolves cgroups. A uprobe patches a specific on-disk file/inode, not "libssl.so.3 wherever a process loads it from": a pod running its own container image (e.g. this repo's `cert-test-server` UBI9 image) ships its own separate copy of `libssl.so.3` baked into that image, a different file with a different checksum from the host's. Tetragon loads and attaches the uprobe cleanly (no error in its logs) — it just never intercepts anything, because the pod's process never executes code from the host's copy of the file. Kernel-level hooks with no userspace-library dependency (`security_socket_bind`/`tcp_connect`, backing `tls-service-tracking.yaml`/`tcp-connect-tls.yaml`) are unaffected by this — they fire regardless of which libssl a process links. There's no per-pod fix yet analogous to the Java agent's `jattach`-into-the-target-process approach (below); for now the test-server's `in-memory-asn1-cert` use case is disabled by default in k8s/OpenShift deployments (`demo.testServer.disabledUseCases`, wired to `TEST_SERVER_DISABLE_USE_CASES`) rather than shown as a broken demo button.

**Does the Java JCA keystore use case (`java-jca-keystore`) work in k8s/OpenShift?**
No — `java-non-fips-cert.yaml`, the TracingPolicy it depends on, only exists for bare metal under `tetragon-policies/experimental/` and was never ported to the Helm chart (`extras/helm/cert-analyzer/templates/policies/` has no Java policy at all). The use case's `jattach` step itself would likely work in-cluster (it targets a specific JVM process directly rather than a fixed library path, avoiding the gap above), but with no matching TracingPolicy loaded there's nothing to catch the `java_cert_agent_write` uprobe event. Also disabled by default in k8s/OpenShift deployments alongside `in-memory-asn1-cert` until a container variant of the policy is written.
...
Mostly, as of the `[port_probe] sni_capture_enabled` option. The kprobe on `tcp_connect`/`security_socket_bind` only gives Tetragon the destination IP:port (L3/L4) — it fires before any TLS bytes, including the ClientHello's SNI extension, are sent, so by default cert-analyzer's own verification handshake in `_probe_tls_endpoint` has no real hostname to send and falls back to the raw destination IP as SNI. Against a single-tenant server this is harmless; against a CDN edge that multiplexes many customer certs by SNI, an IP-string SNI matches no real vhost, so the edge serves its own generic "no SNI match" default certificate instead of the real one the process negotiated — confirmed live on the AWS demo box: `dnf`'s Fastly-fronted mirror traffic probed back as `*.sni-<n>-default.ssl.fastly.net` rather than the mirror's actual certificate.

The fix (connect-probe only, i.e. outbound client connections — bind-probe/server side has no outbound SNI to fix): an `SSL_ctrl(cmd==SSL_CTRL_SET_TLSEXT_HOSTNAME)` uprobe (`agent/analyzer.py`'s `_handle_ssl_ctrl_sni_capture`) captures the real hostname a client's own OpenSSL library is about to send as SNI, and `_probe_tls_endpoint` uses it instead of the raw IP when the capture is fresh (`sni_capture_window_seconds`, default 2s). Two residual gaps remain, both accepted rather than solved: statically-linked/non-OpenSSL clients never call `SSL_ctrl` at all, so they still fall back to the raw-IP behavior; and correlation is by PID alone, so a process with several concurrent outbound TLS connections at once can have hostnames cross-attributed. Also RHEL9-class kernels only — string-typed uprobe args need `bpf_copy_from_user_str` (kernel 6.3+), the same constraint `openssl3-cert-load-rhel8.yaml` already works around elsewhere by omitting string args, so this capability has no RHEL8 build at all. Off by default (`sni_capture_enabled=false`; Helm: `policies.opensslCertLoad.captureConnectSni: false`).
